...
type Node struct {
	...
	score uint32   // the score
	ids   []uint64 // associated ids
}

func (n *Node) Ids() []uint64 {
	return n.ids
}

func (n *Node) Score() uint32 {
	return n.score
}
...
func lookup_node(n *Node, rank int) (id uint64, node *Node) {
	if n == nil {
...
func new_node(score uint32, id uint64, color bool, left, right *Node) *Node {
	n := Node{score: score, color: color, left: left, right: right, size: 1, ids: []uint64{id}}
	return &n
}

//--------------------------------------------------------- Lookup by Rank
// READ-LOCK
func (t *Tree) Rank(rank int) (id uint64, node *Node) {
	return lookup_node(t.root, rank)
}

//--------------------------------------------------------- Lookup by score
func (t *Tree) _lookup_score(score uint32) (rank int, n *Node) {
	n = t.root
...
//---------------------------------------------------------- locate a score & id
// READ-LOCK
func (t *Tree) Locate(score uint32, id uint64) (uint32, *Node) {
...
//---------------------------------------------------------- Insert an element
// WRITE-LOCK
func (t *Tree) Insert(score uint32, id uint64) {
...
// WRITE-LOCK
func (t *Tree) Delete(id uint64, n *Node) {
